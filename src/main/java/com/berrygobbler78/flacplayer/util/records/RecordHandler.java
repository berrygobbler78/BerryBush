                ...
                inStream.filter(path -> path.toString().endsWith(".flac"))
                        .forEach(path -> {
                            try {
                                ...
                                if(artist == null) {
                                    var artPath = ResourceHandler.get(ResourceHandler.ResourceType.ARTIST_ART).getPath() +
                                            File.separator +
                                            FileUtils.makeFolderSafe(artistStr) +
                                            File.separator +
                                            "art.png";
                                    artist = new Artist(artistStr, new ArrayList<>(),
                                            (new File(artPath).exists() ? artPath : new File(URI.create(ImageUtils.getWarningURL())).getPath())
                                    );
                                ...
                                if(album == null) {
                                    var artPath = ResourceHandler.get(ResourceHandler.ResourceType.ALBUM_ART).getPath()+
                                            File.separator +
                                            FileUtils.makeFolderSafe(artistStr) +
                                            File.separator +
                                    ...
                                    var img = ImageIO.read(new File(artPath));

                                    album = new Album(
                                            albumStr,
                                            artist,
                                            new ArrayList<>(),
                                            (img == null || img.getWidth() <= 0 || img.getHeight() <= 0 ?
                                                    new File(URI.create(ImageUtils.getWarningURL())).getPath() : artPath)
                                    );
